...
## Facilitator integration (STE-60 phase-2 wiring)

The contract is wired into the Phase 1 facilitator as `UptoStellarScheme` (`examples/facilitator/src/modules/facilitator/upto/`), a `SchemeNetworkFacilitator` registered alongside the exact scheme. `verify` cross-checks the payload against the requirements (asset, recipient, cap, window) and confirms the buyer's signed authorization simulates cleanly; `settle` submits the settle call with the facilitator-chosen amount and returns the on-chain hash plus the settled amount. End-to-end on testnet through the scheme: verify returned valid, settle moved a partial 300000 of a 1000000 cap to the merchant in tx `8274de6a0fd8f5586325248a3f2987e894addec8a3b119658bb3e58de7ee2759`.

Two integration facts worth recording. First, the buyer must simulate with the facilitator as the transaction source, not itself — when the buyer is the source, Soroban represents its auth as the implicit source-account credential instead of a detached address credential, leaving nothing to sign offline. Second, the facilitator package pinned `@stellar/stellar-sdk` at v15 while `@x402/stellar` (which settles the exact scheme on testnet) uses v14.6; that split made v15 mis-parse the simulation auth XDR ("unknown SorobanCredentialsType member for value 2"), so the facilitator is now aligned to v14.6.
